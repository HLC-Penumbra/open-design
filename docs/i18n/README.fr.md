...
**Points de terminaison de modèles local-first.** Les URL de base BYOK sur `127.0.0.1`/`localhost`, RFC1918 (`10.x`, `172.16–31.x`, `192.168.x`), CGNAT (`100.64–127.x`) et IPv6 ULA (`fd00::/8`) fonctionnent sans configuration. Les link-local (`169.254/16`, `fe80::/10`), `0.0.0.0` et `::` restent refusés — ce sont des erreurs de configuration universelles, pas des hôtes de modèles locaux. La protection des URL de téléchargement reste stricte : un lien de téléchargement contenu dans une réponse de fournisseur ne peut pas pivoter vers le loopback, le RFC1918, le link-local ou le service de métadonnées cloud.
