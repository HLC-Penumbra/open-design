...
**Локальные эндпоинты моделей (local-first).** Базовые URL BYOK на `127.0.0.1`/`localhost`, RFC1918 (`10.x`, `172.16–31.x`, `192.168.x`), CGNAT (`100.64–127.x`) и IPv6 ULA (`fd00::/8`) работают без настройки. Link-local (`169.254/16`, `fe80::/10`), `0.0.0.0` и `::` по-прежнему отклоняются — это повсеместные ошибки конфигурации, а не хосты локальных моделей. Защита download-URL остаётся строгой: ссылка на скачивание из ответа провайдера не может вести на loopback, RFC1918, link-local или сервис метаданных облака.
