...
describe('BYOK draft validation', () => {
  ...
  it('treats a syntactically-valid internal-IP base URL as non-blocking, deferring to the daemon', () => {
    // The Test / model-fetch actions gate on blockingByokDraftIssues; an
    // internal endpoint must not be blocked here or the user can never
    // reach the daemon's authoritative decision.
