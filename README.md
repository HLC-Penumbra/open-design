...
**Local-first model endpoints.** BYOK base URLs on `127.0.0.1`/`localhost`, RFC1918 (`10.x`, `172.16–31.x`, `192.168.x`), CGNAT (`100.64–127.x`), and IPv6 ULA (`fd00::/8`) work without configuration. Link-local (`169.254/16`, `fe80::/10`), `0.0.0.0`, and `::` are still refused — they are universally misconfigurations, not local-model hosts. The asset-download URL guard stays strict regardless, so a download link inside an upstream API response cannot pivot onto loopback, RFC1918, link-local, or the cloud metadata service.
